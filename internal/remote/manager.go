...
func ConnectionErrorDetails(err error) (message, detail string, ok bool) {
	...
	if errors.Is(err, ErrAgentAPIFingerprint) {
		return "Machine API certificate fingerprint mismatch", "Verify the SHA256 TLS certificate fingerprint from the machine's trusted console, replace it in Servers > Reconnect, then reconnect. The endpoint and port remain fixed.", true
	}
