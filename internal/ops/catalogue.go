...
func CommandCatalogue() []domain.CommandDefinition {
	return []domain.CommandDefinition{
		...
		{Action: "service.logs", Parameters: []domain.CommandParameter{pathParam("id", "Service ID or name", ""), queryNumber("tail", "Lines to return", "1 to 1000. Defaults to 200.")}, Description: "Bounded sanitized Fluentd query for a service's logs.", Docker: "SwarmOps Fluentd query", Endpoint: "GET /api/v1/services/{id}/logs", Resource: "service", Title: "Service logs"},
