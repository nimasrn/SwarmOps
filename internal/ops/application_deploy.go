...
func (c *ControlPlane) DeployApplication(ctx context.Context, actor, requestID string, spec ApplicationSpec) error {
	...
	// the application starts, the database it is pointed at exists, is
	...
	c.record(actor, requestID, "application.deploy", "stack/"+spec.StackName(ApplicationNamespace), err, map[string]string{
...
func (c *ControlPlane) PlanApplication(ctx context.Context, spec ApplicationSpec) ([]byte, error) {
	...
	// application already owns, and falls back to the placeholder below when
	...
	rendered, _, err := c.renderApplication(ctx, spec, uris, false)
	return rendered, err
}
...
func (c *ControlPlane) renderApplication(ctx context.Context, spec ApplicationSpec, uris map[string]string, requireDatabases bool) ([]byte, string, error) {
	...
	for _, engine := range spec.Databases {
		...
		if !found {
			...
			if requireDatabases {
				return nil, "", fmt.Errorf("managed %s is not provisioned for %q; deploy the database and the application again", definition.DisplayName, spec.Name)
			}
			// Planning renders a placeholder rather than refusing. The
			// account is created by the deployment itself, so a first-time
...
func (c *ControlPlane) prepareApplicationRouteNetwork(ctx context.Context, spec ApplicationSpec) error {
	...
	return adapter.PrepareRoutingNetwork(ctx, agentcontrol.RoutingNetworkRequest{
		Network:          RouteNetworkName(spec.ServiceDNSName(ApplicationNamespace)),
