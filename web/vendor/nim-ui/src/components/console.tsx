...
export function Panel({
  ...
  const head = title || description || eyebrow || actions
      ...
      {head ? (
        <header className="nim-panel__head">
          <div className="nim-panel__heading">
            ...
            {title ? <h2 className="nim-panel__title">{title}</h2> : null}
...
export function MetricGrid({ children, className, columns = 4, ...props }: MetricGridProps) {
  return (
    <div className={cn('nim-metric-grid', className)} data-columns={columns} {...props}>
      {children}
    </div>
  )
}
