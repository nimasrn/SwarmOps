...
export function EvidenceLedger({
  ...
  if (!observability.coreInstalled) {
    ...
    closers.push({ title: 'Install the metrics collector', detail: 'Turns cluster history from absent into a measured series.', cta: 'Open Collectors', page: 'observability' })
    ...
    closers.push({ title: 'Repair the metrics collector', detail: 'It is installed and silent, which reads as no data rather than as a fault.', cta: 'Open Collectors', page: 'observability' })
  }
  if (!observability.logsEnabled) {
    absent.push({ label: 'Log retention', value: 'unknown', why: 'no log collector is enrolled' })
    closers.push({ title: 'Enrol the log collector', detail: 'Gives retention a number instead of a shrug.', cta: 'Open Collectors', page: 'observability' })
