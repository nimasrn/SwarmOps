...
export function CommandCenter({ cluster, commands, core, onOpen, servers }: CommandCenterProps) {
  ...
  return (
    <Screen
      actions={
        <Inline>
          <Button iconStart="play" onClick={() => onOpen('deploy')} variant="accent">Deploy from source</Button>
          <Button iconStart="plus" onClick={() => onOpen('machines')} variant="secondary">Add a server</Button>
        </Inline>
      }
      meta={
        <Inline gap="tight">
    ...
    >
      ...
      <Columns align="start" template="two-thirds">
        <Panel
          actions={attention.length ? <Button onClick={() => onOpen('runs')} size="sm" variant="secondary">Open runs</Button> : undefined}
      ...
      <Panel
        actions={
          <Inline>
            ...
            <Button onClick={() => onOpen('swarm')} size="sm" variant="ghost">Swarm &amp; placement</Button>
...
function nextStep({ attention, cluster, connectedServer, core, servers }: {
  ...
  if (!servers.length) {
    return { detail: 'No host is enrolled yet. Enrollment is one outbound command on Ubuntu; the agent keeps its own key and waits for you to approve its code.', icon: 'plus', label: 'Connect your first server', page: 'machines' }
  }
  if (!connectedServer) {
    return { detail: 'No agent is answering the controller, so nothing on this screen is current. Diagnostics names the layer that stopped — transport, Docker, or Swarm.', icon: 'link', label: 'Diagnose the connection', page: 'machines' }
  }
  if (attention) {
    return { detail: `${attention.label}. ${attention.detail}`, icon: 'activity', label: 'Review the open decision', page: attention.page }
  }
  if (!cluster) {
    return { detail: 'An agent is connected but this console is not pointed at a Swarm manager. Selection is deliberate: reads and changes stay scoped to one explicit cluster.', icon: 'plus', label: 'Choose a cluster', page: 'machines' }
  }
  ...
  return { detail: 'Controller, agent, Docker, and Swarm are all answering, and no run is waiting on a decision. The cluster is ready for a deployment.', icon: 'play', label: 'Deploy from source', page: 'deploy' }
}
...
function setupSteps({ cluster, servers }: { cluster?: DashboardData; servers: Server[] }): SetupStep[] {
  ...
  const steps = [
    { action: 'Connect a server', id: 'enroll', label: 'Enroll a host and let its agent connect', page: 'machines' as const },
    { action: 'Open host setup', id: 'swarm', label: 'Make the host a Docker Swarm manager', page: 'machines' as const },
    ...
    { action: 'Deploy from source', id: 'deploy', label: 'Deploy something and watch it start serving', page: 'deploy' as const },
    { action: 'Deploy monitoring', id: 'observe', label: 'Collect metrics and traces, so a verdict has evidence behind it', page: 'platform' as const },
  ]
