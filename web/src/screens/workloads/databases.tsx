 ...
 */
export function DatabasesPage({ toast }: { toast: Toast }) {
  ...
  if (error) {
    return (
      <Screen page="databases">
        <Banner tone="danger" title="Managed databases are unavailable">{error}</Banner>
      </Screen>
    )
  }
  if (!databases) {
    return (
      <Screen page="databases">
        <Panel><Spinner label="Reading managed databases" /></Panel>
      </Screen>
    )
  }
  ...
  return (
    <Screen
      about="Three reviewed database stacks ship with SwarmOps. Their Compose definitions are checked-in assets, their passwords are generated Swarm secrets, and each is reachable only on the internal swarmops overlay."
      insights={[
        { hint: 'Engines with a running task on this cluster', icon: 'database', label: 'Running', tone: running.length ? 'success' : 'neutral', value: `${running.length} / ${databases.length}` },
        { hint: stalled.length ? 'Deployed but not serving — no task is running' : 'No deployed engine is missing its task', icon: 'alert', label: 'Deployed but down', tone: stalled.length ? 'danger' : 'success', value: String(stalled.length) },
        { hint: 'Reviewed engines this controller can deploy', icon: 'layers', label: 'Available engines', value: String(databases.length) },
      ]}
      page="databases"
    >
      <Columns>
      ...
      <Panel eyebrow="How your services connect" title="Credentials and placement">
        ...
        <Body size="sm" tone="muted">Attach one of these to an application under Deliver → Deploy from source; SwarmOps injects the connection URI as a mounted secret file.</Body>
      </Panel>
    </Screen>
  )
}
