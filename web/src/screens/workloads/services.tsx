...
export function ServicesPage({
  ...
  return (
    <Screen
      about="Compose and standalone containers are listed under Fleet → Docker resources. Restarts, rollbacks, and scale changes are queued as fixed, audited command shapes."
      actions={<Button iconStart="document" onClick={onOpenLogs} variant="secondary">Open the Logs workspace</Button>}
      insights={[
        { hint: 'Replicated and global services scheduled by Swarm', icon: 'terminal', label: 'Services', value: String(services.length) },
        { hint: unhealthy.length ? 'Fewer tasks running than desired' : 'Every service has its desired tasks', icon: 'alert', label: 'Below desired', tone: unhealthy.length ? 'danger' : 'success', value: String(unhealthy.length) },
        { hint: updating.length ? 'A rolling update is in progress' : 'No rollout is in progress', icon: 'refresh', label: 'Updating', tone: updating.length ? 'accent' : 'neutral', value: String(updating.length) },
        { hint: 'Tasks Swarm reports as running across all services', icon: 'layers', label: 'Running tasks', value: String(services.reduce((total, service) => total + service.runningTasks, 0)) },
      ]}
      page="services"
      width="full"
    >
