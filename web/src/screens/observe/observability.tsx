 ...
 */
export function ObservabilityPage({ nodes, onOpenGateway, onOpenSwarm, status, toast, traefik }: {
  nodes: Node[]
  onOpenGateway: () => void
  onOpenSwarm: () => void
  ...
  return (
    <Screen
      about="One cluster-wide observability stack owns Prometheus, Alertmanager, and Jaeger. Log collection and host probes are separate, explicit global deployments because they inspect every node."
      insights={[
        { hint: status.coreInstalled ? (status.coreHealthy ? 'Prometheus, Alertmanager, and Jaeger are healthy' : 'Docker reports at least one service degraded') : 'Metrics and traces have no collector', icon: 'chart', label: 'Core monitoring', tone: status.coreInstalled ? (status.coreHealthy ? 'success' : 'danger') : 'warning', value: status.coreInstalled ? (status.coreHealthy ? 'Healthy' : 'Degraded') : 'Not installed' },
        { hint: status.logsEnabled ? (status.logsHealthy ? 'Fluentd is collecting container and host output' : 'The pipeline is present but degraded') : 'The Logs screen has nothing to read', icon: 'document', label: 'Log collection', tone: status.logsEnabled ? (status.logsHealthy ? 'success' : 'danger') : 'neutral', value: status.logsEnabled ? (status.logsHealthy ? 'Healthy' : 'Degraded') : 'Disabled' },
        { hint: status.agentInstalled ? (status.agentHealthy ? 'Host CPU, memory, and disk are being sampled' : 'The probe is installed but degraded') : 'Host figures come from the agent inventory only', icon: 'server', label: 'Node inventory', tone: status.agentInstalled ? (status.agentHealthy ? 'success' : 'danger') : 'neutral', value: status.agentInstalled ? (status.agentHealthy ? 'Healthy' : 'Degraded') : 'Optional' },
        { hint: 'Of the three collectors this controller can deploy', icon: 'activity', label: 'Collectors deployed', value: `${collectors} / 3` },
      ]}
      page="observability"
    >
      <Columns>
