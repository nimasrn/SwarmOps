...
export const FIXTURES: Record<string, unknown> = {
  ...
  // The controller's own once-a-minute sample. It used to be `null` because the
  // only screen reading it was unreachable; it now feeds the cluster trend on
  // Home and the disk breakdown on Storage, so the harness has to answer it.
  '/api/v1/insights': {
    capacity: { cpuCores: 24, diskBytes: 960e9, diskUsedBytes: 412e9, memoryBytes: 96e9 },
    configs: 2,
    containers: { paused: 0, running: 9, stopped: 2, total: 11, unhealthy: 1 },
    generatedAt: ago(30),
    networks: { ingress: 1, overlay: 3, total: 6 },
    nodes: { managers: 1, ready: 2, total: 3, unavailable: 1 },
    secrets: 4,
    services: { degraded: 1, desiredTasks: 8, runningTasks: 6, total: 3, unhealthy: 1 },
    storage: {
      buildCacheBytes: 4.1e9, containerWritableBytes: 1.2e9, imageBytes: 22e9, images: 14,
      layersBytes: 22e9, reclaimableBuildCacheBytes: 3.6e9, reclaimableImageBytes: 8.4e9,
      reclaimableVolumeBytes: 2.1e9, unusedImages: 5, unusedVolumes: 2, volumeBytes: 6.3e9, volumes: 7,
    },
    swarm: { autoLockManagers: false, createdAt: ago(2592000), id: 'swarm-1', taskHistoryLimit: 5 },
    tasks: { desired: 8, failed: 2, total: 10 },
  },
  '/api/v1/insights/history': Array.from({ length: 24 }, (_, index) => ({
    at: ago((24 - index) * 60),
    buildCacheBytes: 4.1e9,
    containersRunning: 9 - (index % 3),
    containersStopped: 2,
    containersTotal: 11,
    containersUnhealthy: index > 18 ? 1 : 0,
    diskCapacityBytes: 960e9,
    diskUsedBytes: 400e9 + index * 5e8,
    imageBytes: 22e9,
    nodesReady: 2,
    nodesTotal: 3,
    nodesUnavailable: 1,
    reclaimableBytes: 14e9,
    servicesDegraded: index > 16 ? 1 : 0,
    servicesTotal: 3,
    servicesUnhealthy: index > 20 ? 1 : 0,
    tasksDesired: 8,
    tasksFailed: index > 16 ? 2 : 0,
    tasksRunning: index > 16 ? 6 : 8,
    tasksTotal: 10,
    volumeBytes: 6.3e9,
  })),
  '/api/v1/events': [
    { Action: 'start', Actor: { Attributes: { name: 'production_checkout-api.1' }, ID: 'c1' }, Type: 'container', time: Math.floor(Date.now() / 1000) - 120, timeNano: (Date.now() - 120000) * 1e6 },
    { Action: 'die', Actor: { Attributes: { name: 'production_api-gateway.2' }, ID: 'c2' }, Type: 'container', time: Math.floor(Date.now() / 1000) - 300, timeNano: (Date.now() - 300000) * 1e6 },
    { Action: 'pull', Actor: { Attributes: { name: 'ghcr.io/example/checkout:1.8.4' }, ID: 'i1' }, Type: 'image', time: Math.floor(Date.now() / 1000) - 900, timeNano: (Date.now() - 900000) * 1e6 },
  ],
  '/api/v1/traefik/logs': [],
