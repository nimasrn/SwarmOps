...
The native installer enables automatic updates for its fixed GitHub release
repository and installs a local systemd timer/path unit on Linux or a
...
3. Warden resolves the latest immutable release, downloads the matching
   Agent/Warden bundle and `checksums.txt`, verifies SHA-256, rejects unexpected
   archive entries, stages the candidate, atomically changes `releases/current`,
   restarts the Agent and provisioning helper, and checks localhost health.
   An unhealthy candidate is removed after the previous release is restored.

The Core request has no body and cannot select a repository, release, binary,
shell command, or restart target. Warden defers while a native-agent mutation
is active. `--no-auto-update` disables and removes the installed update
triggers. In that state the update-request endpoint returns `409 Conflict`
with a specific not-configured message; it does not convert the agent to a
generic transport failure. Production Agent installation and updates require
no Git checkout, Go toolchain, Go module proxy, or server-side compilation.
...
For an outbound agent, use the full command generated by **Fleet → Servers**.
It includes `--core-fingerprint SHA256:<64-hex>` so the first request
